...
GitHub Copilotには、Skills、Agent、Hooks など機能がどんどん増えますよね。
便利なのはありがたいのですが、どの場面でどれを使えばいいのか、どう設定すればいいのかがわからず困っていたので整理しました。

### 注釈

- 本記事の内容はAIに調査してもらい、私が内容をチェックしてまとめたものになります。
- 2026/03/02時点の内容です
- ファイルパスはMacOS環境の例です
...
| 機能                      | 設定コマンド                             | 主な用途                                            |
| ------------------------- | ---------------------------------------- | --------------------------------------------------- |
| `copilot-instructions`    | `/init`                                  | プロジェクト全体の規約・方針                        |
| `*.instructions.md`       | `/instructions`                          | 言語・ファイル種別ごとのルール                      |
| `*.prompt.md`             | `/prompts`                               | 繰り返し使う定型タスク                              |
| `*.agent.md`              | `/agents`                                | Plan,Ask,Agent 以外のカスタム専門ロール             |
| `SKILL.md`                | `/skills`                                | `/名前` で呼ぶ or AI が自動選択する専門知識・手順書 |
| `mcp.json`                | `/mcp` (CLIのみ)                         | 外部ツール・API との連携                            |
| `hooks.json`              | `/hooks`                                 | ライフサイクルへの介入                              |
| `settings.json`（VSCode） | `Preferences: Open User Settings (JSON)` | Copilot の動作制御                                  |

### 設定へのアクセス

VSCodeのCopilot Chatにある歯車アイコンから、一通りの機能設定にアクセスできます。
...
## 1. copilot-instructions.md — 全体共通の指示
...
## 7. hooks.json — エージェントのライフサイクルに介入するイベント駆動スクリプト

> **公式ドキュメント**: https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-hooks
> **VS Code 向け**: https://code.visualstudio.com/docs/copilot/customization/hooks

**役割**: エージェントセッションの開始・終了、ツール実行の前後などのイベントに応じてカスタムシェルコマンドを自動実行する仕組み。
...
※VSCodeとCLI固有のフックタイプが存在するのでそちらは公式ドキュメントを確認
...
1. `copilot-instructions.md`から始める
   - `/init` コマンドで雛形を生成し、プロジェクトの技術スタックや規約を記述する
   - これだけでも Copilot の出力品質が大きく変わる

2. `*.instructions.md` で言語やディレクトリ別のルールを追加する
   - 複数言語を扱うプロジェクトでは特に効果的

3. `SKILL.md` で繰り返し作業や専門スキルを定型化する
   - 定型作業、コンポーネント生成、テスト作成、といった特定用途のスキルを自動化する

4. `*.agent.md` で専門エージェントを定義する
   - レビューやドキュメント生成など、特定用途に特化させると精度が向上する

5. `mcp.json` / `hooks.json` で外部連携・自動化を組み込む
